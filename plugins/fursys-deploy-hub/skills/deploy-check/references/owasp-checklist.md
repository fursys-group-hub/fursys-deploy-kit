...
> deploy-check 스킬이 "🔒 보안 심화" 단계에서 코드를 읽고 판단할 때 참조하는 룰셋이다.
> **시크릿/키 탐지·git 이력은 엔진(`fdh-engine`)이 이미 결정적으로 처리한다.** 여기서는 그 위에
> LLM이 코드를 읽고 판단해야 하는 OWASP 배포 시점 실수 항목을 다룬다(시크릿 스캔 중복 금지).
...
| 16 | 인증 미들웨어 예외처리가 다운스트림 호출까지 감쌈 (item R9-6 / G-new-10, **안내성**) | 인증 미들웨어의 `try/except`(또는 `try/catch`)가 **`call_next(request)`(=다운스트림 요청 처리) 까지 감싸면**, 라우트 핸들러에서 난 임의의 런타임 예외가 미들웨어에 삼켜져 **"로그인 필요(401)"로 위장**되고 스택트레이스도 사라진다(비밀번호가 맞는데 "로그인 필요"가 뜨는 추적 불가 증상 — review-guide-checker). 점검: 인증 데코드 `try` 블록 안에 `call_next`/`next()` 호출이 들어있는지. 있으면 예외 처리 범위를 **인증 파싱까지만**으로 좁히고 다운스트림 예외는 통과시키도록 안내(500 은 500 으로 보이게). **verdict 는 막지 않는 안내성 항목** | 중간(안내) |
| 17 | 발송·알림성 앱의 환경분리 부재 (item R9-8 / N-new-8, **안내성 — 업무사고 방지**) | 메일/알림/외부 전송을 하는 앱은 **① 발송이 배포·접속만으로 자동 트리거되는가**(서버에 `cron`/`setInterval`/`schedule` 발송 트리거가 있으면 dogfooding 배포 즉시 외부로 실발송 = 사고), **② 수신자 test-mode 환경분리가 코드에 있는가**(테스트/운영 구분 없이 실제 외부 주소가 하드코딩되면 테스트 발송이 그대로 외부 파트너에게 감)를 본다. 프론트의 `isTestMode=false` 같은 **번들 상수는 신뢰 불가**(브라우저 변조·재빌드 필요) — 발송이 반드시 지나는 **서버 단일 병목**에 test-mode 게이트(`MAIL_TEST_MODE` 기본 true·미설정 시 안전측, 실발송은 `=false` 명시해야만)를 두도록 안내. dogfooding 배포 땐 `MAIL_TEST_MODE` 미주입(=기본 test)으로 외부 발송 차단. **verdict 는 막지 않되, 자동발송 트리거가 있으면 배포 전 반드시 짚는다**(업무 사고 예방) | 중간(안내) |
