...
- `SUMMARY_CARDS` — 치명/높음/중간/낮음 카드 4개. **숫자 = 엔진 `summary` + 심화(owaspFindings) 를 심각도별 합산**(보안 표에 보이는 전체와 일치하도록 — 카드만 0인데 표엔 항목 있는 불일치 방지). 예: `<div class="count-card critical"><div class="n">1</div><div class="l">치명</div></div>` (high/medium/low 동일 패턴). ※ 합산은 **표시용일 뿐**, 게이트 판정(`final`/`security`)·`last-verdict.json` 의 `summary` 는 엔진 값 그대로 둔다.
