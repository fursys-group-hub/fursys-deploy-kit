...
- **`HAS_MANIFEST`** → 이 앱은 화면·기능처럼 **여러 부분으로 올라간다.** 이때는 `references/multiservice.md` 를 읽어 그대로 따른다(서브도메인/URL 계산 → cross-URL 치환 → 의존 순서 정렬 → 부분마다 deploy.sh 호출). 사용자에게는 "이 앱은 두 부분(화면·기능)으로 올라가요"처럼 쉽게만 설명하고, ③에서 받은 **주소 앞부분 1개**로 모든 부분의 주소를 자동 계산한다(부분 개수만큼 묻지 않는다). **'둘 다 / 화면만 / 기능만' 같은 선택지를 제시하지 않는다 — 항상 전체를 의존순서대로(기능 먼저 → 화면) 함께 올린다(권장안 자동 진행).** 게이트(⑤)는 **repo 단위 1회**만 확인한 뒤 모든 부분을 배포한다. 설정값 수집(④)·게이트(⑤)·결과 처리(⑦·⑨)·마무리(⑧)의 규칙은 그대로 적용하되, 배포 실행만 부분마다 반복한다.
