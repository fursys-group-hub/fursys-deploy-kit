...
- **브랜드/공간 선택 — `creatable_teams` 값만 쓴다(절대 지어내지 말 것). 이 값은 `my-apps.sh` 출력에서만 온다 — 사용자에게 "어느 브랜드냐"고 먼저 묻지 말 것(키가 답을 이미 갖고 있다. `my-apps.sh` 를 부르기 전에 브랜드를 묻는 것은 규칙 위반이다):**
  - **`creatable_teams` 가 1개면 — 묻지 말고 그 팀으로 자동 확정한다(질문 금지. 이건 선택이 아니라 이미 정해진 결정이다).** "○○ 공간에 올릴게요"라고 통보만 한다. ⚠️ **보기가 1개뿐인 `AskUserQuestion` 을 띄우는 것은 규칙 위반** — 팀이 하나면 질문 자체를 만들지 않는다.
...
4. **빠진 비밀·키 — 자동생성과 질문을 구분한다(`references/env-resolve.md` 규칙, 모든 앱 공통):**
   ...
   - **사람이 정하는 값**(`ADMIN_PASSWORD` 등 관리자 비번)·**외부 서비스 자격증명**(`*_API_KEY`/`*_TOKEN`/사외 `*_PASSWORD`, Supabase·외부 DB 접속값 등 — `env_plan` 에서 `note:"ask"` 로 분류된 모든 값) → **배포(⑥) 전에 반드시 한 개씩 묻는다(강제 게이트):**
...
printf '%s' "$ENV_JSON" | "$CLAUDE_PLUGIN_ROOT/skills/deploy/scripts/deploy.sh" \
  "fursys-group-hub/<REPO>" "<COMMIT>" "<TEAM>" "<주소앞부분>" "${PORT:-3000}" "" "${VOL_FLAG[@]}"
```
