...
- **등록된 앱만 배포된다(서버 강제).** 사내 서버에는 **AI Hub Playground 에 앱 등록 신청이 된 repo** 만 올라간다. 서버가 신청 기록(신청서의 GitHub 주소 기준)을 보고 없으면 ⑥에서 `409 not_registered` 로 거부한다 — 이 스킬이 우회할 방법은 없고, **신청은 사용자가 화면에서 직접** 해야 한다(⑦-2 로 돕는다). 판정은 **신청이 있는지 없는지만** 본다 — 승인 전(검토중)이어도 접수되어 있으면 배포된다(승인은 보통 배포 뒤에 난다).
...
- **스크립트는 플러그인에 번들돼 있다 — 프로젝트 파일이 아니다.** `deploy.sh`·`status.sh`·`logs.sh`·`common.sh` 는 `$CLAUDE_PLUGIN_ROOT/skills/deploy/scripts/` 아래에 플러그인과 **항상 함께 설치**된다. **현재 프로젝트 폴더엔 없는 게 정상이고, `github-setup` 등 다른 스킬이 만들어 주는 것이 아니다.** 따라서 "이 프로젝트에 deploy.sh 가 없다 / github-setup 으로 생성돼야 한다"고 **절대 단정하지 말 것**(환각 — 실제로는 플러그인 경로에 있다).
...
- **`GATE_NOT_REGISTERED`(409 not_registered)**: **AI Hub Playground 에 이 앱의 등록 신청이 없다.** 사내 서버에는 Playground 에 신청된 앱만 올릴 수 있다. **앱은 만들어지지 않았다.** ⑦-2 를 수행하고 **멈춘다**(재시도하지 않는다 — 신청은 사람이 화면에서 해야 하므로 다시 눌러도 같은 결과다).
