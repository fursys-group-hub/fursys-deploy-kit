...
- **(pgpublish) 배포에 성공하면 Playground 등록이 자동으로 마무리될 수 있다** — 배포 주소·보안 인증 배지·승인까지 서버가 처리한다(⑧에서 결과를 안내). 관리자가 손으로 하던 단계라, 되면 사용자에게 알려 주고 **안 됐다고 해서 문제라고 말하지 않는다**(배포는 이미 성공했고 관리자 수동 승인이 폴백이다).
...
- **(pgpublish) Playground 자동 반영 안내 — 응답에 `playground` 가 있을 때만.** `deploy.sh` 는 결과 코드 다음 줄에 **서버 응답 JSON 전문**을 그대로 출력한다(별도 스크립트 없음). 그 JSON 의 `"playground"` 값에 따라 한 줄만 전한다:
  - `approved` → "**AI Hub Playground 등록도 자동으로 끝냈어요 ✓** 배포 주소가 등록되고, 보안 검토 통과 표시(인증 배지)와 승인까지 처리됐어요. 이제 Playground 목록에서 보여요."
  - `url_updated` → "**Playground 의 배포 주소를 최신으로 맞췄어요 ✓** (이미 승인된 앱이라 승인은 그대로예요.)"
  - `skipped` → "Playground 쪽은 **관리자가 따로 처리 중인 상태**라 자동으로 건드리지 않았어요. 궁금하면 IT본부에 문의해 주세요." (반려·차단 상태를 자동 승인으로 뒤집지 않는다.)
  - **응답에 `playground_url` 이 있으면 그 줄에 이어 링크를 준다** — "내 앱 보기: `<playground_url>`". 사용자가 바로 확인할 수 있는 카탈로그 페이지다. **`skipped` 에는 링크가 오지 않는다**(카탈로그에 안 보이는 상태에 링크를 주면 헛걸음이라 board 가 아예 안 준다). 필드가 없으면 링크 줄을 만들지 않는다 — **주소를 추측해서 짓지 말 것.**
  - **`playground` 필드가 없으면 아무 말도 하지 않는다.** 없다는 것은 (ⓐ 이 기능이 아직 꺼져 있음 / ⓑ 반영에 실패함) 둘 중 하나인데, **어느 쪽이든 배포는 성공했고** 관리자가 화면에서 승인하는 기존 절차가 그대로 돌아간다. 여기서 "실패했어요"를 알리면 배포가 잘못된 것처럼 들려 불필요한 불안만 준다(`GET /registration` 확인 실패를 알리지 않는 것과 같은 원칙 — ③-2 참조).
  ```bash
  # 응답 JSON(위 결과 줄 다음)에서 playground 값만 뽑기 — 없으면 아무 출력 없음
  printf '%s' "$DEPLOY_JSON" | grep -oE '"playground"[[:space:]]*:[[:space:]]*"[^"]*"' | head -n1
  ```
