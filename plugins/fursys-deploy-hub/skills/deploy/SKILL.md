...
- **`REDEPLOYED <app_id> <domain>`**: **이미 만들어진 본인 앱에 다시 배포를 시작했어요**(앱을 새로 만들지 않고 같은 앱에 재시도). 처리·폴링은 `CREATED` 와 **완전히 동일**하다 — 스크립트가 이어서 상태를 폴링하고, 그 결과(`RUNNING`/`DEPLOY_FAILED`/`PENDING`)도 위 `CREATED` 와 똑같이 처리한다(`DEPLOY_FAILED` 면 ⑨로). 사용자에겐 쉬운 말로만 안내한다:
  > "이미 있는 앱에 **다시 배포**를 시작했어요. 앱을 새로 만들지 않고 같은 앱에 다시 올리는 거예요. 잠시만 기다려 주세요."
  - (내부) 200 응답의 `redeployed:true` 로 들어온 경로다. `RUNNING`/`DEPLOY_FAILED`/`PENDING` 안내 문구는 `CREATED` 케이스 것을 그대로 쓴다.
...
- **다음 단계**: "고친 뒤 **'배포 전 검토 해줘'** 를 다시 돌려 통과시키고, 다시 배포하면 돼요."
  - **반드시 함께 전한다(앱 삭제→재생성 악순환 차단):** "**앱을 지우지 마세요.** 고친 뒤 그냥 **'다시 배포해줘'** 라고 하시면 **같은 앱에 새로 시도**합니다(처음부터 다시 만들 필요 없어요)."
  - **언제 무엇으로 다시 올리나:** 코드를 고쳤으면 `git push` 만으로도 자동 재배포돼요. 코드 말고 설정값·포트 같은 걸 바꿨거나 일시적인 실패였다면 **'다시 배포해줘'** 라고 하면 같은 앱에 다시 시도해요(앱을 새로 만들 필요 없음).
