...
- **주소 앞부분 — repo 이름을 기본값으로 추천하고 확인받는다:** ①에서 확인한 repo(`fursys-group-hub/<레포이름>`)의 **`<레포이름>` 을 그대로(쪼개지 말고) 주소 앞부분 기본값으로 제안**한다. 예: repo `cataloglens` → 앞부분 `cataloglens`, repo `nps-dashboard` → 앞부분 `nps-dashboard`. ⚠️ **`-` 로 잘라 `nps` 로 줄이지 말 것 — repo 이름 전체를 그대로 쓴다.** "접속 주소 앞부분은 `<레포이름>` 으로 할게요. 다른 걸 원하시면 한 단어로 알려주세요." 처럼 **추천값을 보여주고**, 사용자가 그대로면 그 값을, 바꾸면 입력값을 쓴다(자유 변경 가능). **후보 메뉴(`AskUserQuestion`)를 여러 개 만들지 말 것 — 기본은 repo 이름 1개 추천 + 자유 변경이다.** 최종 주소는 **`https://{앞부분}.app2.hub.fursys.com`** 이다(운영 2호기 전용 공간 — v4.6 표준, /deploy 커맨드가 `--server coolify-oper-web-2` 를 붙인다). 예: 앞부분 `cataloglens` → **`cataloglens.app2.hub.fursys.com`**. ⚠️ **`.fursys.com` 으로 끝나면 틀린 것**(반드시 `.app2.hub.fursys.com`). **확정 주소를 그대로 보여주고 확인**받는다. "서브도메인"이라는 말은 쓰지 않는다. (자동 배치(/deploy-dev)는 배치 결과에 따라 app1/app2 로 달라진다. **이미 올라가 있는 기존 앱은 예전 주소(`~.group.hub.fursys.com` 등)를 그대로 유지한다** — 재등록해도 주소가 바뀌지 않는다.)
...
- **`--server <이름>`(선택)**: 배포 대상 서버를 지정한다. **표준 `/deploy` 커맨드가 `--server coolify-oper-web-2`(운영 2호기)를 항상 붙인다**(v4.6 — 구 `/deploy2` 는 이 표준화로 제거됨). `/deploy-dev` 는 `--server auto`(자동 배치). 이때 `SERVER_UNKNOWN`(서버 이름 못 찾음)·`SUBDOMAIN_RESERVED`(예약된 주소 앞부분) 결과 코드가 오면 ⑦처럼 쉬운 우리말로 안내한다.
