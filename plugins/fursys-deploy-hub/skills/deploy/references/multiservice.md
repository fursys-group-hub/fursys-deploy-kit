...
> 사용자에게는 "이 앱은 화면(web)과 기능(api) 두 부분으로 올라가요"처럼 쉬운 우리말로만 설명한다. 서비스/서브도메인/env 같은 용어는 노출 금지.
...
- 시작 시 한 줄로 알린다: "이 앱은 두 부분으로 올라가요 — 화면(web)과 기능(api). 주소는 화면 쪽을 기준으로 정하면 됩니다." (이름·역할은 매니페스트의 name 으로 쉽게 풀어 설명)
...
## 9. 진행 상황 안내 (쉬운 우리말)
- 시작: "기능(api)을 먼저 올리고, 그다음 화면(web)을 올릴게요. (화면이 기능 주소를 알아야 해서 순서가 정해져 있어요.)"
- 각 단계 완료: "기능(api) 올라갔어요 → 주소: https://catalog-api.iloom.hub.fursys.com"
