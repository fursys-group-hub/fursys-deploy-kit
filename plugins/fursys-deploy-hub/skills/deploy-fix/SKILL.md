...
| 모드 | 입력 정본 | 재검증 | 진입 |
|---|---|---|---|
| **A. 검토 수정**(기존) | `_engine.json` findings + `last-verdict.json` commit | `/deploy-check`(deploy-check) 재실행 | `/deploy-fix`(인자 없음) |
...
- **`HAS_ARTIFACTS`** → 두 파일을 읽는다:
  ...
  - **(deployfix2) `last-verdict.json.deploy_fixes[]` = 엔진 finding 에 없는 자동수정 가능 문제 목록**(`{type, severity, file, message, aiPrompt}`). deploy-check 가 적어 둔 **두 출처**를 담는다: ① 🚀 배포 준비 문제(`copy-public`·`port`·`start-cmd`·`streamlit-config` 등 — 4)·4-1 점검), ② 🔒 **LLM 심화 보안 발견(`type:"sec-llm"`)** — 3) 보안 심화에서 LLM 이 잡았으나 엔진이 `_engine.json` 으로 못 잡은 보안 문제(소스 박힌 개인계정 평문·DEBUG·CORS `*` 등). **이 목록의 `aiPrompt` 있는 항목도 ③의 자동수정 대상에 포함**한다 — 그래야 리포트가 "deploy-fix 가 고쳐줘요"라 안내한 문제를 실제로 고친다(계약갭 해소). `_engine.json`(엔진 보안 finding)과 **출처만 다를 뿐 같은 자동수정 파이프라인**으로 처리한다. **엔진 finding 이 비어 있어도(`findings:[]`) `deploy_fixes` 에 `sec-llm` 항목이 있으면 자동수정이 동작**한다(과거 `PLAN_EMPTY` 오판 해소). 없거나 `[]` 면 현행과 동일(엔진 보안 finding 만 처리).
...
수정 직후 **`/deploy-check`(deploy-check) 로직을 자동 실행**해 통과 여부를 다시 판정하고, 새 `.md` 리포트·`_engine.json`·`last-verdict.json` 을 갱신한다.
- **deploy-check 스킬의 절차(엔진 실행 → 보안 심화 → 배포 가능성 → 종합 판정 → 산출물 기록 → `.md` 리포트 생성)를 그대로 자동 수행**한다(사용자에게 `/deploy-check` 를 다시 치라고 시키지 않는다 — 여기서 자동으로 돌린다).
  - **(treegate) 이 재검토가 `last-verdict.json` 을 다시 쓸 때, deploy-check 5-1 의 `_fdh_tree` 산출이 그대로 포함되므로 `tree` 도 함께 갱신된다.** deploy-fix(모드 A)는 **커밋하지 않으므로**(현행 유지), 여기 기록되는 `tree` 는 방금 수정한 **작업트리 내용**의 해시다. deploy ①이 그 작업트리를 커밋하면 commit SHA 는 바뀌어도 tree 는 같아, deploy ⑤ 게이트가 재검토 없이 통과시킨다(SHA 무효화 루프 제거).
- **루프 방지(반드시):**
...
- **일부 남음** → "여기까진 제가 자동으로 고쳤어요. 남은 건 사람 판단이 필요해요." + 남은 항목을 ③의 같은 고정 형식으로 보여준다. 자동수정 불가 항목(사람이 정하는 값·외부 자격증명·기록에 남은 시크릿)은 **명확히 구분**해 안내한다:
  ...
  - **(item60) 아키텍처·인증 구조는 "왜 앱 만든 분(앱 오너) 몫인지"를 명시한다** — 자동으로 못 고치는 대표 5종을 뭉뚱그리지 말고 각각 이유를 붙여 안내한다(caution 이 남아 사용자가 "왜 안 끝나지?"로 2라운드를 또 돌리거나 혼란에 빠지지 않게):
    ...
    - **(supapolicy) 사외 데이터 저장소에 데이터를 두는 경우 — 치명·배포 차단**(deploy-check `framework-rules.md §10-2`/§10 판정: 사내 ref 밖 Supabase, Firebase Firestore/Auth/Storage 등) → "회사 자료를 **회사 밖 저장소**에 두고 있어서 **사내 서버에는 이대로 올릴 수 없어요.** 사내 데이터베이스·파일 저장소를 발급받아 옮겨야 풀려요 — **신청은 AI추진팀**, 방법은 여기 있어요: https://ai-library.hub.fursys.com/guides/data-db/supabase . 옮기는 건 앱을 만든 분이 하셔야 해요 — 제가 자동으로 못 고치는 부분이에요."
      - **`blocked` 가 남는 게 정상이다.** 이 항목 때문에 ⑥ 재검토 루프를 돌리지 않는다. "고치면 통과된다"는 인상을 주지 말 것 — 데이터를 옮기기 전에는 통과하지 않는다.
    - **(supapolicy) 데이터베이스를 브라우저에서 직접(주소 + 공개키 방식) 쓰는 경우 — 치명·배포 차단** → "데이터베이스를 **브라우저에서 직접** 쓰고 있어서 **사내 서버에 올릴 수 없어요.** 사내는 **접속정보(Postgres)를 발급**해 드리고 **앱의 서버 쪽 코드에서만** 연결하는 방식만 허용해요. 데이터를 읽고 쓰는 부분을 서버 쪽(API)으로 옮기는 건 앱을 만든 분이 하셔야 해요 — 제가 자동으로 못 고치는 부분이에요. **파일 저장 기능은 그대로 두셔도 돼요.**"
      - **자동으로 하지 말 것:** 공개키를 빈 값/env 로 바꿔 finding 만 없애지 않는다(값만 비었을 뿐 그 코드는 여전히 브라우저에서 DB 를 부르므로 앱이 조용히 깨진다 — item61 과 같은 함정). **`.storage.from(`/`/storage/v1/object/`(파일 저장) 쪽 키·코드는 건드리지 않는다** — 사내도 그 방식을 쓰므로 정상이다. `sb_publishable_…`/`anon` 키는 **공개 전제 값이라 소스에 있어도 보안 문제가 아니다** — 없애거나 숨기려 들지 않는다. **단 구형 JWT anon 키(`eyJ…`)가 엔진 `SEC-07`(치명)에 걸려 배포가 막힌 경우**에는 ⑤-3 의 값 보존 절차(로컬 `.env`+`.gitignore`+`.env.example`)로 설정값으로 빼 준다 — 엔진이 그 파일을 안 읽어 통과한다. **이건 임시 우회다.** 통보에 **"이 옛날 키는 2026년 말 폐기 예정이라, 새 방식 키(`sb_publishable_...`)로 바꾸시는 게 좋아요(값만 바꾸면 동작은 그대로예요)"** 를 함께 넣는다. **DB 위반이 해결된 것도 아니다**(구조는 그대로) — 묶어서 "해결됐다"고 보고하지 말 것.
  - **이 다섯 종류는 자동수정·재시도 대상이 아니다** — ⑥의 재검토 루프를 이것들 때문에 반복하지 않는다(③ 제외 규칙과 동일). "제가 고칠 수 있는 건 다 했고, 남은 건 제가 손댈 수 없는 종류"임을 분명히 한다.
