...
- **자동수정 대상:**
  ...
  - **(deployfix2) `aiPrompt` 가 있는 배포준비 문제**(`last-verdict.json.deploy_fixes[]` — `type` 이 `copy-public`·`port`·`start-cmd`·`next-build-env`·`volume-perm`·`streamlit-config`·`secrets-to-env`·`static-folder`·`healthcheck-ipv4`(구 `nginx-healthcheck`)·`dockerfile` 등). 이것도 같은 자동수정 파이프라인으로 ⑤에서 고친다(public 폴더 생성·포트 맞춤·`.streamlit/config.toml` 생성·설정값 폴백·정적 폴더 좁히기 등). **소스 최소 변경 원칙**: 코드 변경 없이 설정파일/Dockerfile 로 풀 수 있으면 그 경로를 우선한다(⑤-4 참조).
